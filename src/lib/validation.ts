...
/**
 * แก้ "เเ" (สระเอสองตัว) ให้เป็น "แ" ตัวเดียว ตั้งแต่ตอนบันทึก
 *
 * สระ เ ในภาษาไทยเขียนไว้หน้าพยัญชนะ สองตัวติดกันจึงวาดออกมาเหมือน แ
 * ทุกพิกเซล แต่เก็บเป็นคนละอย่าง — "แก๊ส" ยาว 4 ตัว ส่วน "เเก๊ส" ยาว 5 ตัว
 *
 * ⚠️ "เเ" ไม่ใช่ภาษาไทยที่ถูกต้องเลยแม้แต่กรณีเดียว
 *    สระ เ ต้องมีพยัญชนะตามหลังเสมอ สอง เ ติดกันแปลว่าตัวแรกไม่มีพยัญชนะ
 *    ซึ่งเป็นไปไม่ได้ในภาษาไทย มันจึงเป็นการพิมพ์ แ ผิดเสมอ 100%
 *    ไม่มีคำไหนที่ตั้งใจเขียนแบบนั้น การแก้ให้จึงทำลายคำที่ถูกต้องไม่ได้เลย
 *
 * ที่มาของมันมีจริง — เครื่องพิมพ์ดีดไทยกับคอมพิวเตอร์ยุคแรกบางรุ่นไม่มีปุ่ม
 * แ คนจึงพิมพ์ เเ แทน ข้อความแบบนั้นยังลอยอยู่ในเน็ตเยอะจนถึงทุกวันนี้
 * ส่วนบนมือถือคือกดปุ่ม เ สองครั้ง
 *
 * ⚠️ ต้องแก้ที่ตอนบันทึก ไม่ใช่ตอนแสดงผล
 *    เคยจะรวมสองแบบนี้ตอนรวมยอดในหน้าเจาะประเภท เจ้าของร้านค้านว่าแบบนั้น
 *    คือเก็บผิดไว้แล้วแกล้งทำเป็นถูก เขาจะไม่มีวันรู้ว่าตัวเองพิมพ์สองแบบ
 *    แก้ตรงนี้แทน ข้อมูลในฐานจึงมีแบบเดียวตั้งแต่แรก ไม่มีอะไรให้ต้องรวมทีหลัง
 *    (หน้าเจาะประเภทยังรวมเฉพาะชื่อที่ตรงกันทุกตัวอักษรเหมือนเดิม)
 *
 * อยู่ชั้นเดียวกับ trim ที่ตัดช่องว่างหัวท้ายให้อยู่แล้ว — จัดการสิ่งที่
 * เป็นไปไม่ได้หรือไม่มีความหมาย ก่อนเก็บลงฐาน
 */
const fixThaiSaraAe = (v: string) => v.replaceAll("เเ", "แ");

/** ข้อความไทยที่คนพิมพ์เอง — ตัดช่องว่างหัวท้ายและแก้ เเ เป็น แ */
const thaiText = () => z.string().trim().transform(fixThaiSaraAe);

const nameSchema = thaiText().pipe(
  z.string().min(1, "ใส่ชื่อด้วย").max(120, "ชื่อยาวเกินไป"),
);
...
const optionalText = (max: number) =>
  z
    ...
    .nullish()
    // แก้ เเ เป็น แ เหมือนช่องชื่อ ดูเหตุผลที่ fixThaiSaraAe
    .transform((v) => (v ? fixThaiSaraAe(v) : null));
...
export const createTransactionSchema = z.object({
  ...
  title: thaiText().pipe(
    z.string().min(1, "ใส่ชื่อรายการด้วย").max(200, "ชื่อรายการยาวเกินไป"),
  ),
  amount: amountSchema,
