...
> **อัปเดตฐานข้อมูลที่มีอยู่แล้ว** — รัน [supabase/upgrade.sql](../supabase/upgrade.sql) ที่ SQL Editor
> ไฟล์เดียวสำหรับทุกการอัปเดต ไม่ต้องไล่ดูว่าเคยรันอันไหนไปแล้ว
> ทุกคำสั่งมี `if not exists` จึงรันซ้ำได้เสมอ และไม่แตะข้อมูลเดิมแม้แต่แถวเดียว
>
> ฐานที่สร้างใหม่จาก `supabase/schema.sql` มีทุกอย่างอยู่แล้ว ไม่ต้องรัน
